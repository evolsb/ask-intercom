...
from .logging import ANALYTICS_DIR, ERRORS_DIR, LOGS_DIR, SESSIONS_DIR
...
class LogAnalyzer:
    ...
    def find_sessions_with_errors(self, hours: int = 24) -> List[Dict[str, Any]]:
                ...
                sessions_with_errors.append(
                    {
                        ...
                        "errors": len(
                            [
                                log
                                for log in session_info.get("logs", [])
        ...
        return sorted(
            sessions_with_errors, key=lambda x: x.get("start_time", ""), reverse=True
        )
    ...
    def get_system_summary(self) -> Dict[str, Any]:
        ...
        analytics_status = {
            ...
            "directory_writable": self.analytics_dir.exists()
            and self.analytics_dir.is_dir(),
            "log_files_count": len(list(self.logs_dir.glob("*.jsonl")))
            if self.logs_dir.exists()
            else 0,
            "session_files_count": len(list(self.sessions_dir.glob("*.json")))
